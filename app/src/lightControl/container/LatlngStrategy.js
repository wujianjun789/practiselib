...
class LatlngStrategy extends Component {
  ...
  componentWillUnmount() {
    ...
    collapseClick=(id, key, data) => {
      ...
      this.setState({[`${key}Data`]:data.setIn([getIndexByKey(data, key == 'strategy' ? 'key' : 'id', id),
        'collapsed'], !getProByKey(data, key == 'strategy' ? 'key' : 'id', id, 'collapsed'))});
    }
        ...
        <div className={`container-fluid sidebar-info ${sidebarInfo.collapsed ? 'sidebar-collapse' : ''}`}>
          ...
          <div className="switch-container">
            ...
            <button className="btn btn-primary pull-right" 
              disabled={selectItem.status == this.formatIntl('app.status.started')} 
              onClick={() => this.switchStrategy('start')}>{this.formatIntl('button.start')}</button>
            <button className="btn btn-danger pull-right" 
              disabled={selectItem.status == this.formatIntl('app.status.paused')} 
              onClick={() => this.switchStrategy('pause')}>{this.formatIntl('button.pause')}</button>
          </div>
          {
              ...
              <div className="panel panel-default group-tast-record">
                ...
                <div className={'panel-body ' + (sidebarInfo.propertyCollapsed ? 'collapsed' : '')}>
                  ...
                  <Page className={ 'page ' + (page.get('total') == 0 ? 'hidden' : '') } 
                    pageSize={ page.get('pageSize') } current={ page.get('current') } total={ page.get('total') }
                ...
                <div className="panel panel-default strategy-info">
                  ...
                  <div className={'panel-body ' + (sidebarInfo.parameterCollapsed ? 'collapsed' : '')}>
                    <div className="form-group">
                      ...
                      <div className="input-container">
                        <input type="text" className="form-control" value={selectItem.hasOwnProperty('excuteTime')
                          ? selectItem.excuteTime : ''} disabled="disabled"/>
                      </div>
                    </div>
                    <div className="form-group">
                      <label>{this.formatIntl('app.brightness')}</label>
                      <div className="input-container">
                        <input type="text" className="form-control" value={selectItem.execution 
                          ? selectItem.execution.light : ''} disabled="disabled" />
                      </div>
                    </div>
                    <div className="form-group">
                      ...
                      <div className="input-container">
                        <input type="text" className="form-control" value={selectItem.excuteOffset
                          ? selectItem.excuteOffset : ''} disabled="disabled" />
                      </div>
                    </div>
                    <button className="btn btn-primary pull-right" onClick={this.taskRecordPopup}>
                      {this.formatIntl('app.task.record')}</button>
                  </div>
                </div>
