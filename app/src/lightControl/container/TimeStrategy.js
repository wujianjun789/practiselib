...
class TimeStrategy extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      deviceList: {
        ...
        valueKey: 'name',
        options: [
          /*{id:1, name:"test灯"},{id:2, name:"test显示屏"}*/
        ]
      },
    ...
    this.taskColumns = [
      {
        id: 0,
        field: 'create',
        title: this.formatIntl('app.task.create.time')
      },
      { id: 1, field: 'time', title: this.formatIntl('app.task.execute.time') },
      {
        id: 2,
        field: 'result',
        title: this.formatIntl('app.task.execute.result')
      }
    ];
  }
      ...
      if (parent.name.indexOf(search.get('value')) > -1) {
        ...
        if (parent.plans) {
          parent.plans.map(item => {
            ...
            // item.timeRange =
            // dateStringFormat(item.start) + '-' + dateStringFormat(item.end);
            item.timeRange =
              this.timeFormatter(item.start) +
              ` - ` +
              this.timeFormatter(item.end);
            result.push(item);
          });
        }
      } else {
        ...
        if (!item.hasOwnProperty('plans')) {
          getPlanStatus(item.id, res => {
            ...
            if (len2 == len1) {
              result.length > 0 &&
                this.setState(
                  { selectItem: result[0], [key]: Immutable.fromJS(result) },
                  () => {
  ...
  initTaskData = data => {
    let result = data.map(item => {
      item.create = item.create
        ? momentDateFormat(
            getMomentDate(item.create, 'YYYY-MM-DDTHH:mm:ss.SSSZ'),
            'YYYY-MM-DD HH:mm:ss'
          )
        : '';
      item.time = item.time
        ? momentDateFormat(
            getMomentDate(item.time, 'YYYY-MM-DDTHH:mm:ss.SSSZ'),
            'YYYY-MM-DD HH:mm:ss'
          )
        : '';
      return item;
  ...
  collapseHandler = id => {
    this.setState({
      sidebarInfo: Object.assign({}, this.state.sidebarInfo, {
  ...
  collapseClick = (id, key, data) => {
    ...
    this.setState({
      [`${key}Data`]: data.setIn(
        [
          getIndexByKey(data, key == 'strategy' ? 'key' : 'id', id),
          'collapsed'
        ],
  ...
  taskRecordPopup = () => {
    ...
    actions.overlayerShow(
      <TaskRecordPopup
        className="task-record-popup"
        intl={this.props.intl}
        title={this.formatIntl('app.task.record')}
  ...
  switchStrategy = key => {
    ...
    actions.overlayerShow(
      <ConfirmPopup
        tips={
          this.formatIntl(
            key == 'start' ? 'app.status.start' : 'app.status.pause'
          ) +
          this.formatIntl(selectItem.plans ? 'app.group' : 'app.strategy') +
          '?'
        }
        iconClass={key == 'start' ? 'icon_popup_start' : 'icon_popup_pause'}
        cancel={() => {
          actions.overlayerHide();
        }}
        confirm={() => {
  ...
  render() {
    ...
    return (
      <Content
        className={`control-time-strategy ${
          sidebarInfo.collapsed ? 'collapse' : ''
        }`}
      >
        ...
        <div
          className={`container-fluid sidebar-info ${
            sidebarInfo.collapsed ? 'sidebar-collapse' : ''
          }`}
        >
          <div
            className="row collapse-container"
            role="presentation"
            onClick={() => this.collapseHandler('collapsed')}
          >
            <span
              className={
                sidebarInfo.collapsed ? 'icon_horizontal' : 'icon_vertical'
              }
            />
          </div>
            ...
            <div className="panel-strategy">
              {/* <div className="panel panel-default">
                ...
                >
                  ...
                  <div className="form-group">
                    <label>
                      {this.formatIntl('app.strategy.retryInterval')}
                    </label>
                    <div className="input-container">
                      <input
                        type="text"
                        className="form-control"
                        value={selectItem.retryInterval}
                        disabled="disabled"
                      />
                    </div>
                  </div>
              ...
              <div className="panel panel-default strategy-info">
                <div
                  ...
                  onClick={() => {
                    !sidebarInfo.collapsed &&
                      this.collapseHandler('parameterCollapsed');
                  }}
                >
                  <span className="icon_control" />
                  {this.formatIntl('app.strategy.property')}
                  <span className="icon icon_collapse pull-right" />
                </div>
                <div
                  className={
                    'panel-body ' +
                    (sidebarInfo.parameterCollapsed ? 'collapsed' : '')
                  }
                >
                  <div className="form-group">
                    ...
                    <div className="input-container">
                      <input
                        ...
                        className="form-control"
                        value={
                          selectItem.excuteTime ? selectItem.excuteTime : ''
                        }
                        disabled="disabled"
                      />
                    </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-container">
                      <input
                        ...
                        className="form-control"
                        value={
                          selectItem.execution ? selectItem.execution.light : ''
                        }
                        disabled="disabled"
                      />
                    </div>
              ...
              {/* <div className="panel panel-default device-info">
                <div
                  ...
                  onClick={() => {
                    !sidebarInfo.collapsed &&
                      this.collapseHandler('devicesCollapsed');
                  }}
                >
                  <span className="icon_device_list" />
                  {this.formatIntl('app.strategy.devices')}
                  <span className="icon icon_collapse pull-right" />
                </div>
                <div
                  className={
                    'panel-body ' +
                    (sidebarInfo.devicesCollapsed ? 'collapsed' : '')
                  }
                >
                  <div className="header">
                    <span>{`${this.formatIntl('sysOperation.include')}：${
                      selectItem.devices ? selectItem.devices.length : 0
                    }${this.formatIntl('sysOperation.devices')}`}</span>
                  </div>
                  <Table
                    className="selectedDevices"
                    collapsed
