...
export default class LightStatistics extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            // deviceState: {id:'1', domainId:'1', deviceTotal:'100', deviceOnlineRunning:'90', deviceOnlineError:'3', deviceOffline:'7'},
    ...
    getEnergyDataByIdAndType(id, type) {
        ...
        getEnergy(id,type,dateTime,(response)=>{
            // console.log("getEnergyresponse:", response);
            let {data} = this.state.energyFilt;
            let {totalEnergy} = this.state;
            totalEnergy = 0;
            if (response) {data = []}
            for (var key in response) {
                totalEnergy = totalEnergy + response[key].value;
                let dataItem = {};
                    dataItem.x = response[key].dateTime;
                    dataItem.y = response[key].value;
                data.push(dataItem);
            }
            this.setState({energyFilt:Object.assign({},this.state.energyFilt,{data}),totalEnergy:totalEnergy},()=>{
            })
        });
    }
    ...
    getStatusDataByDomainId(id) {
        getDomainStatusByDomainId(id, (response)=>{
            ...
            for (var i = 0; i < response.length; i++){
                ...
                // let valueName = response[i].variable;
                // domainStatus.valueName && domainStatus.valueName = response[i].value;
            }
            domainStatus.domainId = response[0].domainId;
            this.setState({domainStatus:domainStatus})
            // data = this.state.deviceState;
    ...
    energySelect(event) {
        ...
        this.setState(Object.assign({},{energyList},{type}), ()=>{
            let domainId = this.state.domainList.id;
            let type = this.state.type;
            // console.log("domainandtype:", domainId, type)//success
            this.getEnergyDataByIdAndType(domainId, type)
        });
    }
                        ...
                        <div className=" panel">
                            ...
                            <div className="panel-body statecircle1 ">
                                ...
                                <div className="stateInfo">
                                    <div className="fontbold font">域： <span>{`${this.state.domainList.value}`}</span></div>
                                    <div className="fontbold font">设备数：<span>{`${domainStatus.deviceTotal?domainStatus.deviceTotal:'未读取'}`}</span></div>
                                    <div className="dot color1"></div><div className="font">在线正常数：<span>{`${domainStatus.deviceOnlineRunning?domainStatus.deviceOnlineRunning:'未读取'}`}</span></div>
                                    <div className="dot color2"></div><div className="font">在线故障数：<span>{`${domainStatus.deviceOnlineError?domainStatus.deviceOnlineError:'未读取'}`}</span></div>
                                    <div className="dot color3"></div><div className="font">离线数：<span>{`${domainStatus.deviceOffline?domainStatus.deviceOffline:'未读取'}`}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="col-sm-4">
                        <div className=" panel">
                            ...
                            <div className="panel-body statecircle2">
                                ...
                                <div className="stateInfo">
                                    <div className="fontbold font">域： <span>{`${this.state.domainList.value}`}</span></div>
                                    <div className="fontbold font">计划数：<span>{`${domainStatus.planTotal?domainStatus.planTotal:'未读取'}`}</span></div>
                                    <div className="dot color1"></div><div className="font">成功计划数：<span>{`${domainStatus.palnSuccess?domainStatus.palnSuccess:'未读取'}`}</span></div>
                                    <div className="dot color2"></div><div className="font">失败计划数：<span>{`${domainStatus.planFail?domainStatus.planFail:'未读取'}`}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
