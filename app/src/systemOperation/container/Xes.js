...
export class Xes extends Component {
  ...
  componentWillMount() {
    ...
    getModelData(this.state.model, () => {
      if (this.mounted) {
        ...
        this.setState({
          modelList: Object.assign({}, this.state.modelList, {options: 
            getModelTypesById(this.state.model).map((type) => {
  ...
  domainHandler(e) {
    ...
    case 'sys-add': {
      ...
      overlayerShow(<CentralizedControllerPopup popId="add" className="centralized-popup" 
        title={this.props.intl.formatMessage({id:'sysOperation.addDevice'})} model={model}
    ...
    case 'sys-update': {
      ...
      overlayerShow(<CentralizedControllerPopup popId="edit" className="centralized-popup" 
        title={this.props.intl.formatMessage({id:'sysOperation.xes'})} model={model}
    ...
    case 'sys-delete':
      overlayerShow(<ConfirmPopup tips={this.props.intl.formatMessage({id:'delete.device'})} 
        iconClass="icon_popup_delete" cancel={ this.popupCancel } confirm={ this.popupConfirm }/>);
      break;
    case 'sys-dataOrigin':
      overlayerShow(<DataOriginPopup className="dataOrigin-popup" intl={this.props.intl} sensorTypeList={sensorTypeList}
        type={data.type} overlayerHide={overlayerHide} onConfirm={(data, type) => updateDataOrigin(data, type)}/>);
      break;
    default:
      break;
    }
  ...
  render() {
    const {collapse, deviceCollapse, dataOriginCollapse, page, search, selectDevice, domainList, data} = this.state;
    return <Content className={'offset-right ' + (collapse ? 'collapsed' : '')}>
      ...
      <SideBarInfo mapDevice={selectDevice} collapseHandler={this.collapseHandler}
        className={(deviceCollapse ? 'deviceCollapse ' : '') + (dataOriginCollapse ? 'dataOriginCollapse' : '')}>
        ...
        <div className="panel panel-default device-statics-info dataOrigin">
          <div className="panel-heading" role="presentation"
            onClick={() => { !collapse && this.collapseHandler('dataOriginCollapse'); }}>
            <span className="icon_db"></span>{this.props.intl.formatMessage({id:'sysOperation.dataOrigin'})}
            <span className="icon icon_collapse pull-right"></span>              
          </div>
          <div className={'panel-body domain-property ' + (dataOriginCollapse ? 'collapsed' : '')}>
            <span className="domain-name">{`${this.props.intl.formatMessage({id:'sysOperation.include'})}:${
              selectDevice.data.length} ${this.props.intl.formatMessage({id:'sysOperation.dataOrigins'})}`}</span>
            <button id="sys-dataOrigin" className="btn btn-primary pull-right" onClick={this.domainHandler} 
              disabled={data.size == 0}>{this.props.intl.formatMessage({id:'button.edit'})}
            </button>
          </div>
        </div>
