...
import { getPoleAssetsListByPoleId } from '../../api/pole.js';
...
export class sysConfigSmartLight extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      /*  This state save all equipments
       *  whether they were added in pole or not.
       */
      allEquipmentsData: [],
  ...
  /* Declare functions
     ...
     * when componenetWillMount,we call this function to provide DomainList to be chosen.
     */
  initDomainList(data) {
  ...
  initAssetList(data) {
    let list = data.filter(item => item.extendType === 'pole').map((asset, index) => {
      ...
      // First, need to judge the domainList has already exists;
      ...
      /* By each pole's ID,request this pole's all asset model.Counting each asset than setState.
             *  There is one thing should be recognized --- this callback function is asynchronousFunction.
    ...
    return ( <div id="sysConfigSmartLight">
      <Content className={ 'offset-right ' + (collapse ? 'collapsed' : '') }>
        ...
        <SideBarInfo mapDevice={ selectDevice } collpseHandler={ this.collpseHandler }>
          <SiderBarComponet onClick={ this.editButtonClick } disabled={ !initSelectDeviceName } name={ initSelectDeviceName } />
        </SideBarInfo>
      </Content>
    </div>
    );
