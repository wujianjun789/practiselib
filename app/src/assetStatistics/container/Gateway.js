...
export class Gateway extends Component {
  ...
  searchResult(data) {
    let list = data.map(item => {
      ...
      // list.push(Object.assign({id:item.id, extendType:item.extendType, 
  ...
  render() {
    ...
    return (
      <Content className={'offset-right ' + (collapse ? 'collapsed' : '')}>
        ...
        <SideBarInfo  mapDevice={selectDevice} collpseHandler={this.collapseHandler} className={(deviceCollapse ? 'deviceCollapse ' : '')}>
          <div className="panel panel-default device-statics-info">
            ...
            <div className="panel-body view">
              <div className="circle1">
                <Pie data={{type:'NOISE', val:total}} width={width} height={height} 
                  color="#E6BC00" className="noise" range={[0, total]}></Pie>
              </div>
              <div className="circle2">
                <Pie data={{type:'TEMPS', val:normal, unit:'%'}} width={width} height={height} 
                  color="#E6BC00" className="temps" range={[0, total]}></Pie>
              </div>
            </div>
          </div>
        </SideBarInfo>
