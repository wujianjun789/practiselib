...
import {
  getProjectsByPlayerId,
  getProjectPreviewById,
  getCurrentIdActive,
  applyProjectOnPlayer
} from '../../api/mediaPublish';
...
export class MediaPublishScreen extends Component {
  ...
  getCurrentProjects = () => {
    const id =
      this.state.currentDevice && this.state.currentDevice.extend.player;
    getProjectsByPlayerId(id, res => {
      ...
      this.setState(
        { playScheme: newPlayScheme, currentPlan: newPlayScheme[0] },
        () => {
  ...
  handleSelectDomain = e => {
    ...
    this.setState(
      {
        currentDomain: domainList[e.target.selectedIndex],
        page: { ...page, current: 1 }
      },
      this.initDeviceData
    );
  ...
  handleViewDevice = e => {
    ...
    actions.overlayerShow(
      <PreViewPopup
        title="显示屏预览"
        data={{
          url: 'http://localhost:8080/images/smartLight/screen_test.png'
        }}
        onCancel={() => {
  ...
  handlePlanManage = () => {
    ...
    actions.overlayerShow(
      <ProjectPopup
        title="方案管理"
        data={{ playerId: currentPlayerId, applyProjectList: newPlayScheme }}
        onConfirm={data => {
          actions.overlayerHide();
          this.getCurrentProjects();
        }}
...
export default connect(mapStateToProps, mapDispatchToProps)(MediaPublishScreen);
