...
class TimeStrategy extends Component {
  ...
  addHandler=() => {
    ...
    actions.overlayerShow(<StrategyPopup intl={this.props.intl} title={this.formatIntl('app.add.strategy')} type="0"
      onConfirm={(data) => {
        data.type = 0;
        addStrategy(data, () => {
          this.requestSearch();
  ...
  tableEdit=(rowKey) => {
    ...
    actions.overlayerShow(<StrategyPopup isEdit intl={this.props.intl} title={this.formatIntl('app.edit.strategy')} data={row.toJS()} type="0"
      onConfirm={(data) => {
        data.id = row.get('id');
        data.type = 0;
        updateStrategy(data, () => {
  ...
  onChange=(id, value) => {
    if (id == 'time') {
      this.setState({selectItem:Object.assign({}, this.state.selectItem, {excuteTime:value})});
    } else {
  ...
  editStrategy=() => {
    ...
    updateStrategy({id:selectItem.id, execution:selectItem.execution, 
      excuteTime:selectItem.excuteTime.hour()*3600 + selectItem.excuteTime.minute()*60 + selectItem.excuteTime.second()});
  }
  ...
  render() {
    const {search, selectedDevicesData, allDevicesData, strategyData,
      sidebarInfo, selectItem, lightList, allDevices} = this.state;
