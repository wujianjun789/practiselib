...
export default class LatlngStrategyPopup extends Component {
  ...
  onChange(event) {
    ...
    if (id == 'device') {
      ...
      switch (id) {
      case 'name':
        if (value && !NameValid(value)) {prompt = {hidden:false, text:this.formatIntl('mediaPublish.prompt')};}
        break;
      case 'sunriseTime':
      case 'sunsetTime': 
        if (value && !numbersValid(value)) {prompt = {hidden:false, text:this.formatIntl('app.input.numbers')};}
        if (value && value > 1440) {prompt = {hidden:false, text:this.formatIntl('app.exceed.maximum.support')};}
        break;
      default:
        break;
      }
  ...
  render() {
    ...
    let {name, device, sunrise, sunset, sunriseTime, sunsetTime, prompt, deviceList, brightnessList, screenSwitchList}
     = this.state;
    const disabled = !!((sunriseTime === '' || sunsetTime === '' || deviceList.length == 0));
    ...
    return (
      <Panel className={className} title={title} footer={footer} closeBtn={true} closeClick={this.onCancel}>
        <div className="form-group">
          <label htmlFor="name" className="control-label" title={this.formatIntl('app.strategy.name')}>
            {this.formatIntl('app.strategy.name')}</label>
          <div className="form-group-input">
            <input type="text" className="form-control" id="name" placeholder={this.formatIntl('app.latlng.strategy')}
              value={name} maxLength={16} onChange={this.onChange}/>
        ...
        <div className="form-group">
          <label htmlFor="sunriseTime" className="control-label" title={this.formatIntl('app.sunrise.time.difference')}>
            {this.formatIntl('app.sunrise.time.difference')}</label>
          <div className="form-group-input">
            <input type="text" className="form-control" id="sunriseTime" 
              placeholder={this.formatIntl('app.input.minute')} value={sunriseTime} maxLength={16}
        ...
        <div className="form-group">
          <label htmlFor="sunsetTime" className="control-label" title={this.formatIntl('app.sunset.time.difference')}>
            {this.formatIntl('app.sunset.time.difference')}</label>
          <div className="form-group-input">
            <input type="text" className="form-control" id="sunsetTime" 
              placeholder={this.formatIntl('app.input.minute')} value={sunsetTime} maxLength={16}
              onChange={this.onChange}/>
