                        ...
                        <ul className="group-list">
                            {
                                groupList.map((item, index)=>{
                                    return item.name.indexOf(search)>-1?<li className={item.id===groupId?"active":""} key={index} value={item.name} onClick={()=>this.searchSubmit(item)}>{item.name}</li>:""
                                })
                            }
                            {
                                (search && !getObjectByKeyObj(groupList,'name',search)) && <li onClick={this.addGroup}><span className="glyphicon glyphicon-plus"></span>{`创建组“${search}”`}</li>                                
                            }                           
                        </ul>
                    </div>
                </div>
