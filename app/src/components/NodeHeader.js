    ...
    render() {
        ...
        return (
            <div className="row tree-node">
                ...
                {node.get('isAdd')?<span className='tree-node-add'>已添加</span>:<span className='glyphicon glyphicon-plus tree-node-add' onClick={()=>this.props.onClick(node.get('id'))}></span>}
            </div>
        )
    }
}